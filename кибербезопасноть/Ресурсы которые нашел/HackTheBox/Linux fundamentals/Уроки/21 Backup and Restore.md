...
#### Install Rsync
        ...
        shellsession
`august852@htb[/htb]$ sudo apt install rsync -y`
...
#### Rsync - Backup a local Directory to our Backup-Server
        ...
        shellsession
`august852@htb[/htb]$ rsync -av /path/to/mydirectory user@backup_server:/path/to/backup/directory`
...
We can also add additional options to customize the backup process, such as using compression and incremental backups. We can do this like the following:
        ...
        shellsession
`august852@htb[/htb]$ rsync -avz --backup --backup-dir=/path/to/backup/folder --delete /path/to/mydirectory user@backup_server:/path/to/backup/directory`
...
#### Rsync - Restore our Backup
        ...
        shellsession
`august852@htb[/htb]$ rsync -av user@remote_host:/path/to/backup/directory /path/to/mydirectory`
...
#### Secure Transfer of our Backup
        ...
        shellsession
`august852@htb[/htb]$ rsync -avz -e ssh /path/to/mydirectory user@backup_server:/path/to/backup/directory`
...
First, we generate a key pair for our user.
        ...
        shellsession
`august852@htb[/htb]$ ssh-keygen -t rsa -b 2048`
...
Follow the prompts to specify the location (default is `~/.ssh/id_rsa`) and optionally provide a passphrase (leave it empty for no passphrase). Then, we need to copy our public key to the remote server.
        ...
        shellsession
`august852@htb[/htb]$ ssh-copy-id user@backup_server`
